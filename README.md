...
- The pointer leaving the strip collapses it after 0.4 seconds. If the leave event is missed (menu bar, another app), a pointer check still collapses once the cursor is outside the notch and the strip. Pin Touch bar keeps it open until you unpin it from the status menu or right-click → Unpin on the strip.
...
The right wing shows a speaker. Hovering it reveals a volume slider anchored to that wing and does not open the Touch Bar. Scroll on the wing or the slider changes system volume (CoreAudio). Up and right raise volume; down and left lower it. With natural scrolling on, two fingers toward the top of the trackpad (away from you) raise volume and the bar fills upward toward 100%; two fingers toward you lower it and the bar empties toward 0%. A rightward scroll raises it and a leftward scroll lowers it. Click or double-click mutes. At 0% or while muted, the wing and the slider show a crossed-out speaker; otherwise the speaker is the ordinary one and the slider shows the percent. With the strip open, two-finger scroll over the Control Strip brightness button (the sun beside the speaker) changes built-in display brightness. Brightness uses that same mapping: up and right raise it, down and left lower it. The click still opens the system brightness control. Scroll on the mirrored volume button (the speaker beside that sun) changes system volume with that same mapping: up and right raise it, down and left lower it. That click still opens the system volume control. The notch wing is unchanged. Each slot stays while the strip is open and that patch of the picture stays lit, so a hover highlight does not cancel the scroll. A dark patch, or a sun that was never found beside the speaker, does not change brightness or volume. The expanded strip is the adaptive bar itself. If the stream cannot attach, the strip becomes a short message and a Try Again button instead of fake stand-in controls.
...
5. Leaving the strip collapses it after about 0.4 seconds. Moving back onto the center cancels the collapse. Pin Touch bar skips that collapse until you unpin it (status menu or right-click → Unpin).
...
7. Click the status item for Show/Hide, Display, Position, Pin Touch bar, Theme (including Invisible), Size, Open at Login, Copy Diagnostics, and Quit TouchBarPill. Hovering Theme, Size, Position, or Pin Touch bar previews that look until the pointer leaves; a click saves it. Right-click (or Control-click) the expanded strip for Unpin (when pinned) and Quit. Hide is remembered. The status-item icon is still the capsule with three dots. Focus stays on the left wing.

The notch stays on the display you pick. It does not follow the pointer onto another screen. If that display is unplugged, it moves to the built-in display, or the main display, and returns when the saved display is back. Drag the collapsed notch along its attached edge; it cannot leave the display. Top/bottom placements stay flush with that edge; left/right sit mid-height on the bezel. On a notched MacBook display, top placement still meets the physical top; an unpinned expand sits just below the menu bar so the camera notch does not cover the controls. With Pin Touch bar on and Position at Top center, the open strip always sits immediately under the menu bar (`visibleFrame` top), including while that menu row is only hovered, so Apple, app menus, and status items stay clickable.
...
- Pin Touch bar, in the status menu. While pinned, hover-leave does not collapse.
