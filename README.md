...
- The pointer leaving the strip collapses it after 0.4 seconds. Pin expanded keeps it open until you unpin it from the status menu or right-click → Unpin on the strip.
...
- There is no Dock icon and no Preferences window. A status item has Show/Hide, Display (when more than one screen is attached), Position, Pin expanded, Theme (Black, Graphite, Soft accent, and Invisible), Size S/M, Start/Pause/Resume Focus, Reset Focus, Open at Login, Copy Diagnostics, and Quit.

The right wing shows a speaker. Hovering it reveals a volume slider anchored to that wing and does not open the Touch Bar. Scroll on the wing or the slider changes system volume (CoreAudio). Click or double-click mutes. The slider shows the percent, or a mute mark. Brightness is not reimplemented. The expanded strip is the adaptive bar itself. If the stream cannot attach, the strip becomes a short message and a Try Again button instead of fake stand-in controls.
...
7. Click the status item for Show/Hide, Display, Position, Pin expanded, Theme (including Invisible), Size, Focus, Open at Login, Copy Diagnostics, and Quit TouchBarPill. Right-click (or Control-click) the expanded strip for Unpin (when pinned) and Quit. Hide is remembered. The status-item icon is still the capsule with three dots.
...
TouchBarPill/ZonePolicy.swift       Collapsed zones and hover-expand rule
